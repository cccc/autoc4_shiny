class AutoC4Presets {
    init(autoc4, options) {
        return this.autoc4 = autoc4, $("body").on("click", "[data-preset-topic]", (function(e) {
            var $this = $(this);
    ...
    create_button(room, preset) {
        return $("<button>", {
            ...
            "data-preset-topic": "global" === room ? "preset/set" : "preset/" + room + "/set",
    ...
    onMessage(autoc4, message) {
        ...
        if ("list" === room) {
            ...
            for (let element of document.querySelectorAll("div.preset-pane[data-room]")) {
                ...
                for (var pre of presets) this.create_button(r, pre).insertBefore(marker);
            }
        }
        let marker = document.querySelector('div.preset-pane[data-room="' + room + '"] > .preset-pane-room-marker');
        for (var pre of presets) this.create_button(room, pre).insertBefore(marker);
    }
    onConnect(autoc4, o) {
